...
public class WaterDrone : MonoBehaviour
{
            ...
            for (int x = -1; x <= 1; x++) // 搜索八个方向
            {
                for (int y = -1; y <= 1; y++)
                {
                    ...
                    if (temp_x >= 0 && temp_x < 32 && temp_y >= 0 && temp_y < 32)
                    {
                        ...
                        if (temp.cultivated && !temp.wOccupied && temp.HumidityValue <= 0)
                        {
                            temp.wOccupied = true;
                            res_x = temp_x;
                            res_y = temp_y;
    ...
    IEnumerator moveToDoThings(int targetID) //移动到特定LandChunk的中央
    {
        ...
        while (true)
        {
            ...
            {
                speed = 0f;
                waterLandChunk(targetID);
                farm.FarmMatrix[Mathf.FloorToInt(targetID / 32), targetID % 32].wOccupied = false;
    ...
    public void waterLandChunk(int chunkID)
    {
        farm.waterLandChunk(chunkID);
    }
}
